...
public:
    ...
    /**
     * @brief Loads graph edges and user tags from data files.
     * @param edgeFile Path to the edge list file (e.g., "1 2").
     * @param tagFile Path to the tags file (e.g., "1 tagA tagB").
     */
    void load_data(const string& edgeFile, const string& tagFile);

    /**
     * @brief Saves the current network state (edges and tags) back to the files.
     */
    ...
    /**
     * @brief Removes an undirected friendship edge between two users.
     */
    void remove_friendship(int userA, int userB);

    /**
     * @brief Adds a new user to the network with their tags.
     * If user ID already exists, this will overwrite their tags.
     * @param userID The new user's ID.
     * @param tags A set of strings representing the user's tags.
     */
    void add_user(int userID, const unordered_set<string>& tags);

    /**
     * @brief Checks if a user ID exists in the network.
    ...
    /**
     * @brief Gets a user's friend list by constant reference.
     * @param userID The user's ID.
     * @return A const reference to the user's friend set.
     * Returns a static empty set if user does not exist.
     */
    const unordered_set<int>& get_friends(int userID) const;

    /**
     * @brief Gets a user's tag list by constant reference.
     * @param userID The user's ID.
     * @return A const reference to the user's tag set.
     * Returns a static empty set if user does not exist.
     */
    const unordered_set<string>& get_tags(int userID) const;

    /**
     * @brief Gets a list of all users in the network.
     * @return A const reference to the set of all user IDs.
     */
    const unordered_set<int>& get_all_users() const;

    /**
     * @brief Gets the degree (friend count) of a user.
     * @param userID The user's ID.
     * @return The number of friends, or 0 if user does not exist.
     */
